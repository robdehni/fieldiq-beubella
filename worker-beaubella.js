  ...
  async fetch(req, env, ctx) {
    ...
    async function verifyUserCredentials(userId, pin) {
      ...
      return {
        ...
        territory: normalizeTerritoryShared(rec.fields["Territory"])
      };
    }
    ...
    if (req.method === "GET" && url.pathname === "/get-visits") {
      if (!(await checkRateLimit("get-visits", req))) return json({ error: "Too many requests. Please wait a moment and try again." }, 429);
      const _authToken = getBearerToken(req);
    ...
    if (req.method === "GET" && url.pathname === "/get-accounts") {
      if (!(await checkRateLimit("get-accounts", req))) return json({ error: "Too many requests. Please wait a moment and try again." }, 429);
      const _authToken = getBearerToken(req);
      const _authSession = await verifySession(_authToken, env.SESSION_SECRET);
      if (!_authSession) return json({ error: "Invalid or expired session. Please sign in again." }, 401);
    ...
    if (req.method === "GET" && url.pathname === "/get-watchlist") {
      if (!(await checkRateLimit("get-watchlist", req))) return json({ error: "Too many requests. Please wait a moment and try again." }, 429);
      const _authToken = getBearerToken(req);
      const _authSession = await verifySession(_authToken, env.SESSION_SECRET);
      if (!_authSession) return json({ error: "Invalid or expired session. Please sign in again." }, 401);
      if (_authSession.role !== "Manager") return json({ error: "Manager access required." }, 403);
...
Keep the whole answer under 55 words.`;
    ...
    if (url.pathname === "/save-actions") {
      try {
        ...
        const body = await req.json();
        const { visitRecordId, accountId, accountName, actionText, assignedRepRecordId } = body;
        if (!visitRecordId || !accountName || !actionText) {
          return json({ error: "visitRecordId, accountName and actionText required" }, 400);
        }
        // Default: the action belongs to whoever is actually authenticated.
        // A manager may explicitly assign to a different rep; a non-manager
        // cannot — their own verified identity always wins, regardless of
    ...
    if (url.pathname === "/save-visit") {
      ...
      try {
        ...
        // Who actually made this visit is a fact about the authenticated
    ...
    if (url.pathname === "/save-account-update") {
      ...
      try {
        ...
        if (!_session) return json({ error: "Invalid or expired session. Please sign in again." }, 401);
        const body = await req.json();
        const { accountId, accountName, updateText } = body;
        const createdBy = _session.displayName;
        const createdByUserId = _session.userId;
    ...
    if (url.pathname === "/acknowledge-note") {
      try {
        ...
        const _session = await verifySession(_token, env.SESSION_SECRET);
        if (!_session) return json({ error: "Invalid or expired session. Please sign in again." }, 401);
        // NOTE: this only confirms the caller is authenticated, not that
        // the note actually belongs to them. Real ownership verification
        // would require resolving noteId -> its Action ID -> that action's
        // Assigned Rep, then comparing against _session.recordId — left
        // as a known, explicit gap rather than rushed in here.
        const body = await req.json();
        const { noteId } = body;
